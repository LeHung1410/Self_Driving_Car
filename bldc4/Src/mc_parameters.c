...
const ESC_Params_t ESC_ParamsM1 =
{
  ...
  .Ton_max =  ESC_TON_MAX,                      /*!<  Maximum ton value for PWM (by default is 1800 us) 910*/
  .Ton_min =  ESC_TON_MIN,                      /*!<  Minimum ton value for PWM (by default is 1080 us) 520 */
  .Ton_arming = ESC_TON_ARMING,                 /*!<  Minimum value to start the arming of PWM */
  .Ton_Dir = (ESC_TON_MAX + ESC_TON_MIN)/2,     // 715
  .delta_Ton_max = ESC_TON_MAX - ESC_TON_MIN,
  .delta_Ton_dir = (ESC_TON_MAX - ESC_TON_MIN)/2,
  .speed_max_valueRPM = MOTOR_MAX_SPEED_RPM,    /*!< Maximum value for speed reference from Workbench */
  .speed_min_valueRPM = 200,                   /*!< Set the minimum value for speed reference */
  .motor = M1,
};
